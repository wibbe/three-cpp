...
namespace three {
  ...
  void addPlane(Geometry * geom, float width, float depth, int segmentsWidth, int segmentsDepth, Color const& color)
  {
    ...
    for (int y = 0; y < segmentsDepth + 1; ++y)
      ...
      {
        geom->vertices.push_back(Vector3(x * segmentWidth - halfWidth, 0.0f, y * segementDepth - halfDepth));
        geom->normals.push_back(Vector3(0, 1, 0));
        geom->texCoord0.push_back(Vector2(x / (float)segmentsWidth, y / (float)segmentsDepth));
        geom->colors.push_back(color);
      }
    ...
    for (int y = 0; y < segmentsDepth; ++y)
      ...
      {
        uint16_t a = index + x + (segmentsWidth + 1) * y;
        uint16_t b = index + x + (segmentsWidth + 1) * (y + 1 );
        uint16_t c = index + (x + 1) + (segmentsWidth + 1) * (y + 1);
        uint16_t d = index + (x + 1) + (segmentsWidth + 1) * y;

        geom->faces.push_back(Face(a, b, c));
        geom->faces.push_back(Face(c, d, a));
      }
  }
